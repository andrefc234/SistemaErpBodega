...
console.log(updatePedido)
 ...
 <div><Table striped bordered hover responsive><thead>
  ...
  <tbody>
    {dta.map(d => {
        ...
        <tr>
          ...
          <td>{d.fechaPromotoria}</td>
            <td>{activeEditar ? <Button onClick={() => setactiveEditar(false) } >regresar</Button>:<Button onClick={() => setactiveEditar(true)} >editar</Button>}</td>
    ...
    <Button onClick={fetchdtaH}>Ver</Button>
    </Table>
    </div>
    ...
    useEffect(() => {
     ...
     <Form.Group className="mb-3">
       ...
       <Form.Group className="mb-3">
        ...
        <Form.Text className='text-white' type='text' controlId='precio' id= {`precio${count}`}>{p.precio}</Form.Text>
        <br/>
